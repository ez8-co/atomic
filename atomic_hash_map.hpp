/*
  atomic_hash_map -- Lock-Free Hash Map port from folly::AtomicUnorderedInsertMap for C++.
...
namespace atomic {
  ...
  typedef struct ConstIterator : public std::iterator<std::bidirectional_iterator_tag, value_type> {
    ConstIterator(const atomic_hash_map& owner, IndexType slot)
      : owner_(owner)
      , slot_(slot)
    {}
  ...
  private:
    const atomic_hash_map& owner_;
    IndexType slot_;
  } const_iterator;

  friend ConstIterator;

  explicit atomic_hash_map(size_t maxSize,
    ...
    : allocator_(alloc)
  {
    ...
    if (capacity < maxSize || capacity > avail) {
      throw std::invalid_argument(
        "atomic_hash_map capacity must fit in IndexType with 2 bits "
        "left over");
    }
  ...
  std::pair<const_iterator,bool> insert(const K& key, const V& value) {
    return findOrConstruct(key, &atomic_hash_map::copyCtor<V>, &value);
  }
  ...
  /// updating values inserted into an atomic_hash_map<K,
  /// MutableAtom<V>>.  This relies on atomic_hash_map's guarantee
